...
// Função para "subir" o elemento até a posição correta no heap.
void heapifyUp(vector <pair <double, double> >& heap, unordered_map<double, double>& mapa_idx, int index) {
    while (index > 0) {
        int index_pai = (index - 1) / 2;
        if (heap[index].first < heap[index_pai].first) {
            swap(heap[index], heap[index_pai]);
            mapa_idx[heap[index].second] = index;
            mapa_idx[heap[index_pai].second] = index_pai;
            index = index_pai;
        } else {
            break;
        }
...
// Função para "descer" o elemento até a posição correta no heap.
void heapifyDown(vector <pair <double, double> >& heap, unordered_map<double, double>& mapa_idx, int index) {
    int size = heap.size();
    while (true) {
        ...
        if (esquerda < size && heap[esquerda].first < heap[menor].first) {
            menor = esquerda;
        }

        if (direita < size && heap[direita].first < heap[menor].first) {
            menor = direita;
        }
        ...
        if (menor != index) {
            ...
            mapa_idx[heap[index].second] = index;
            mapa_idx[heap[menor].second] = menor;
            index = menor;
        } else {
            break;
        }
...
void insert(vector <pair <double, double> >& heap, unordered_map<double, double>& mapa_idx, int valor, int idx_original) {
    heap.push_back(std::make_pair(valor, idx_original));
    mapa_idx[idx_original] = heap.size() - 1;
    heapifyUp(heap, mapa_idx, heap.size() - 1);
}

// Função para extrair o menor elemento do heap.
pair<double, double> extractMin(vector <pair <double, double> >& heap, unordered_map<double, double>& mapa_idx) {
    if (heap.size() == 0) {
    ...
    if (!heap.empty()) {
        mapa_idx[heap[0].second] = 0;
        heapifyDown(heap, mapa_idx, 0);
    }

    mapa_idx.erase(minElement.second);
    return minElement;
}

// Função para diminuir a chave de um elemento no heap.
void decreaseKey(vector <pair <double, double> >& heap, unordered_map<double, double>& mapa_idx, int idx_original, int novo_valor) {
    int idx_heap = mapa_idx[idx_original];
    heap[idx_heap].first = novo_valor;
    heapifyUp(heap, mapa_idx, idx_heap);
}

// Função para imprimir o heap.
void printHeap(const vector <pair <double, double> >& heap) {
    for (const auto& elem : heap) {
        cout << "(" << elem.first << ", idx: " << elem.second << ") ";
    }
    cout << endl;
}
...
void distancia_peso_heap(int s){
    int minimo, idx;
    vector <pair <double, double> > minHeap;
    unordered_map <double, double> mapa_idx;
    ...
    //Inicia MinHeap.
    for (int i = 0; i < n; i++){
        insert(minHeap, mapa_idx, distancia_dijkstra[i], i);
    }

    for (int total = 0; total < n; total++){
        auto minElement = extractMin(minHeap, mapa_idx); //Extrai menor elemento do heap.
        idx = minElement.second;
        ...
        if (opcao == 0){
            ...
            for (int i = 0; i < n; i++){
                if (matriz_peso[idx][i] != infinito){
                    if (distancia_dijkstra[i] > distancia_dijkstra[idx] + matriz_peso[idx][i]){
                        distancia_dijkstra[i] = distancia_dijkstra[idx] + matriz_peso[idx][i]; //Atualiza com menor distância.
                        decreaseKey(minHeap, mapa_idx, i, distancia_dijkstra[idx] + matriz_peso[idx][i]); //Modifica chave no heap.
                        pai[i] = idx + 1; //Constrói árvore de busca.
                        niveis[i] = niveis[idx] + 1;
                    }
                }
            ...
            for (int i = 0; i < lista_peso[idx].size(); i++){
                if (distancia_dijkstra[lista_peso[idx][i][0] - 1] > distancia_dijkstra[idx] + lista_peso[idx][i][1]){
                    distancia_dijkstra[lista_peso[idx][i][0] - 1] = distancia_dijkstra[idx] + lista_peso[idx][i][1]; //Atualiza com menor distância.
                    decreaseKey(minHeap, mapa_idx, lista_peso[idx][i][0] - 1, distancia_dijkstra[idx] + lista_peso[idx][i][1]); //Modifica chave no heap.
                    pai[lista_peso[idx][i][0] - 1] = idx + 1; //Constrói árvore de busca.
                    niveis[lista_peso[idx][i][0] - 1] = niveis[idx] + 1;
                }
            }
